...
export const jobPostService = {
  getAll: async (filters?: any): Promise<ApiResponse<JobPostType[]>> => {
    const response = await apiClient.get("/job-posts", { params: filters });
  ...
  update: async (id: string, data: Partial<UpdateJobPostData>): Promise<ApiResponse<JobPostType>> => {
    const response = await apiClient.put(`/job-posts/${id}`, data);
    return response.data;
  },
  delete: async (id: string): Promise<ApiResponse<void>> => {
    const response = await apiClient.delete(`/job-posts/${id}`);
    return response.data;
  },
  apply: async (id: string): Promise<ApiResponse<JobPostType>> => {
    const response = await apiClient.post(`/job-posts/${id}/apply`);
    return response.data;
  },
  approve: async (id: string, appId: string): Promise<ApiResponse<JobPostType>> => {
    const response = await apiClient.put(`/job-posts/${id}/applications/${appId}/approve`);
    return response.data;
  },
  reject: async (id: string, appId: string): Promise<ApiResponse<JobPostType>> => {
    const response = await apiClient.put(`/job-posts/${id}/applications/${appId}/reject`);
    return response.data;
  },
};
